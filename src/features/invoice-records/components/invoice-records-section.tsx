...
export function InvoiceRecordsSection({ invoiceId, recordTypes, records, error, saved }: { invoiceId: number; recordTypes: ClientInvoiceRecordType[]; records: InvoiceRecordView[]; error?: string; saved?: string }) {
  ...
  return <section id="invoice-records" class="grid gap-3">
    ...
    {records.map((record) => <RecordCard invoiceId={invoiceId} record={record} />)}
...
function RecordCard({ invoiceId, record }: { invoiceId: number; record: InvoiceRecordView }) {
  return <article class="app-card rounded-lg p-4"><div class="mb-3 flex items-center gap-2"><h3 class="font-semibold">{record.recordTypeName}</h3>{record.purpose === "nfse" ? <span class="badge badge-outline badge-sm">NFS-e</span> : null}</div>
    <form class="grid gap-3" hx-post={`/invoices/${invoiceId}/records/${record.id}`} hx-target="#invoice-records" hx-swap="outerHTML">
      {record.definitions.fields.map((definition) => <RecordField definition={definition} value={record.values[definition.key]} />)}
      <div class="flex justify-end"><button class="btn btn-primary btn-sm">Save information</button></div>
    </form>
    {record.definitions.attachments.length ? <div class="mt-4 border-t border-base-300/60 pt-3"><h4 class="mb-2 text-xs font-semibold uppercase tracking-wide text-base-content/50">Attachments</h4>{record.definitions.attachments.map((definition) => { const attached = record.attachments.filter((item) => item.attachment.definitionKey === definition.key && !item.file.supersededByFileId); return <div class="mb-3"><div class="mb-1 flex justify-between text-xs"><span>{definition.label}{definition.required ? " *" : ""}</span><span class="text-base-content/45">{attached.length}/{definition.maximumCount}</span></div>{attached.map(({ file }) => <a class="link mr-3 text-xs" href={`/invoices/${invoiceId}/records/files/${file.id}`}>{file.originalFilename ?? "Download"}</a>)}{attached.length < definition.maximumCount ? <form class="mt-2 flex gap-2" enctype="multipart/form-data" hx-post={`/invoices/${invoiceId}/records/${record.id}/attachments/${definition.key}`} hx-target="#invoice-records" hx-swap="outerHTML"><input type="file" name="file" class="file-input file-input-sm min-w-0 flex-1" accept={definition.acceptedTypes.join(",")} required/><button class="btn btn-ghost btn-sm">Upload</button></form> : null}</div>; })}</div> : null}
  </article>;
}
