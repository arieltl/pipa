...
export function listTemplates(engine?: string): PdfTemplate[] {
  return db
    ...
    .where(
      engine
        ? and(eq(pdfTemplates.engine, engine), isNull(pdfTemplates.archivedAt))
        : isNull(pdfTemplates.archivedAt),
    )
    .orderBy(asc(pdfTemplates.name))
    .all();
}
export function getTemplate(id: number): PdfTemplate | null {
  return (
    db.select().from(pdfTemplates).where(eq(pdfTemplates.id, id)).get() ?? null
  );
}
export function getRevision(id: number): PdfTemplateRevision | null {
  return (
    db
      .select()
      .from(pdfTemplateRevisions)
      .where(eq(pdfTemplateRevisions.id, id))
      .get() ?? null
  );
}
export function getBuiltinClassic(): PdfTemplate | null {
  return (
    db
      .select()
      .from(pdfTemplates)
      .where(eq(pdfTemplates.stableKey, "builtin-react-classic"))
      .get() ?? null
  );
}
export function getActiveCurrentRevision(
  templateId: number,
): PdfTemplateRevision | null {
  const template = db
    .select()
    .from(pdfTemplates)
    .where(
      and(eq(pdfTemplates.id, templateId), isNull(pdfTemplates.archivedAt)),
    )
    .get();
  return template?.currentRevisionId
    ? getRevision(template.currentRevisionId)
    : null;
}
...
export function insertHtmlTemplate(values: {
  ...
  return insertTemplate({ ...values, engine: "gotenberg-html" });
}

export function insertTemplate(values: {
  name: string;
